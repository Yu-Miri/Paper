...
- Loss
    ...
    - 예측된 Background Inpainting Image(O_s^)와 예측된 Fushion Image(O_t)는 **GAN loss**(생성된 입력 텐서의 로그값 평균을 계산하고 음수를 취하여 Loss 계산)와 **L2 loss**(정답값 입력텐서와 생성한 입력 텐서 간 차이의 제곱을 계산하여 평균을 구하는 L2 loss)로 최적화한다.
